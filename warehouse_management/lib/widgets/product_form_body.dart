...
class ProductFormBody extends StatefulWidget {
  ...
  final Function(ProductFormResult) onSubmit;
...
class _ProductFormBodyState extends State<ProductFormBody> {
  ...
  late final TextEditingController _wholesaleMinQtyController;
  late final TextEditingController _minStockLevelController;
  late final TextEditingController _vatPercentController;
  ...
  void initState() {
    ...
    _wholesaleMinQtyController = TextEditingController();
    _minStockLevelController = TextEditingController();
    _vatPercentController = TextEditingController();
    _warrantyDurationController = TextEditingController();
    _discountValueController = TextEditingController();

    // Pre-fill data in edit mode
    if (widget.mode == ProductFormMode.edit && widget.initialProduct != null) {
      _prefillFormData(widget.initialProduct!);
    } else if (widget.preselectedGroup != null) {
      _selectedCategory = widget.preselectedGroup;
    }
  }

  void _prefillFormData(Product product) {
    _nameController.text = product.name ?? '';
    _salePriceController.text = (product.cost ?? 0).toString();
    _stockQtyController.text = (product.quantity ?? 0).toString();
    _purchasePriceController.text = (product.cost ?? 0).toString();
    _detailsController.text = product.description ?? '';
    _selectedCategory = product.group;
    _selectedUnit = 'ইউনিট';

    // Stock alert
    if (product.stockAlertEnabled == true) {
      _stockAlertEnabled = true;
      _minStockLevelController.text = (product.minStockLevel ?? 0).toString();
    }

    // Images
    if (product.image != null && product.image!.isNotEmpty) {
      _imagePaths = [product.image!];
    }
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _buildLabeledTextField(
            label: widget.mode == ProductFormMode.edit ? 'বর্তমান মজুদ' : 'বর্তমান মজুদ আছে',
            required: false,
          ...
          if (widget.mode == ProductFormMode.create) const SizedBox(height: 16),
          _buildLabeledTextField(
            label: 'ক্রয়মূল্য',
            required: false,
      ...
      child: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              // Barcode Scan Button (only in create mode)
              if (widget.mode == ProductFormMode.create) ...[
                SizedBox(
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text(
                    widget.mode == ProductFormMode.edit ? 'আপডেট করুন' : 'সেভ করুন',
  ...
  Widget _buildDropdownField({
    ...
    // Build items list - include the current value if it's not in the list
    ...
    final String? dropdownValue = (value != null && dropdownItems.contains(value))
        ? value
        : dropdownItems.first;
